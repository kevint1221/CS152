...
%type <expr> multiplicative_exp MultVar Begin_params StaWhi bool_exp StaDo StaCon MultDec StaRet alternativeElse StaFor identCases
...
Program:        FUNCTION ident SEMICOLON Begin_params MultDec END_PARAMS BEGIN_LOCALS END_LOCALS BEGIN_BODY MultStat END_BODY
{
    ...
    |       FUNCTION ident SEMICOLON Begin_params END_PARAMS BEGIN_LOCALS MultDec END_LOCALS BEGIN_BODY MultStat END_BODY
{
    string tmp= "func " + string($2.result_id);
    identifiers.push(tmp);
    counting++;
...
StaRet:        RETURN expression         {string tmp; tmp.append($2.code);tmp.append("ret"); tmp.append($2.result_id);tmp.append("\n"); $$.result_id=strdup(tmp.c_str()); $$.code=strdup(tmp.c_str()); };
...
term:          SUB terCases         {  }
        ...
        |      ident L_PAREN identCases R_PAREN           { string tmp;  tmp.append($3.code); tmp.append("param"); tmp.append($3.result_id); tmp.append("\n"); tmp.append(". "); tmp.append(temporaries.top()); tmp.append("\n"); tmp.append("call ");tmp.append($1.result_id); tmp.append(", "); tmp.append(temporaries.top()); tmp.append("\n");  $$.result_id = strdup($1.result_id); $$.code=strdup("");};

identCases:   expression        { code << $1.code; }
        |      expression COMMA identCases  { string tmp; tmp.append($1.code); tmp.append($3.code); $$.code = strdup(tmp.c_str()); }
        |                                       {};
